...
var maxJobs = numOfJobsShown + 30;

$(".jobListingDiv").on("click", ".loadMoreJobs", function(){
    for (i = numOfJobsShown; i <= maxJobs; i++){
            var listingDate = "/Date("+jobsArr[i].dateAdded+")/";
            var daysOld = (moment(listingDate).diff(moment(), "days") * -1);
            if (daysOld == 0) {
                freshness = "Added today";
